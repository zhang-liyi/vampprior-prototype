...
parser.add_argument('--collapse_channels',
                       default=False, 
                       type=lambda x: (str(x).lower() == 'true'))
parser.add_argument('--reduce_datapoints',
                       default=False, 
                       type=lambda x: (str(x).lower() == 'true'))
...
parser.add_argument('--epsilon', type=float, default=0.2,
                    help='percentage of noise added')
parser.add_argument('--rm_labels', type=int, default=0,
                    help='number of classes to remove')
...
def run(args, kwargs):
    ...
    model_name = args.dataset_name + '_' + args.model_name + '_' + args.prior + '(K_' + str(args.number_components) + ')' + '_wu(' + str(args.warmup) + ')' + \
        '_z1_' + str(args.z1_size) + '_z2_' + str(args.z2_size) + '_noise_' + str(args.add_noise) + str(args.random_noise) + str(args.epsilon) + '_' + str(args.rm_labels)
    ...
    args_as_json = vars(args)
    json_object = json.dumps(args_as_json, indent=4)
    with open(dir+"parameters.json", "w") as outfile:
    ...
    elif args.model_name == 'dcganvae':
        from models.DCGANVAE import VAE
    else:
